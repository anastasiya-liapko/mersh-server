...
$next_order['']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-cogs"></i>\');
				};
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">ID, Название</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Атрибуты".' </h2>
...
".(function_exists("processTD")?processTD("
	<div class='genesis-item-property '>
		...
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=attributes' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span>
	</div>", $item, "Название"):"
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=name&sort_order='. ($next_order['name']) .'\' class=\'sort\' column=\'name\' sort_order=\''.$sort_order['name'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['name'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
...
".(function_exists("processTD")?processTD("
		<div class='genesis-item-property '>
			...
			<div class='text-center genesis-button-container'>
				<a href='attribute_values.php?attribute_id={$item["id"]}' class='btn btn-primary btn-genesis '>
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM attributes WHERE id=?",[$id]);
			$item = $item[0];
		}
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Атрибуты".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `attributes` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO attributes (`name`) VALUES (?)";
	...
	$actions['edit_execute'] = function()
	{
		...
		if(!$skip)
		{
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE attributes SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM attributes WHERE id=?", [$id]);
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM attributes main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM attributes main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM attributes main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
	...
	echo masterRender("Атрибуты", $content, 0);
