<?php
  ...
  0 => '',
	...
	$actions[''] = function()
	{
   		...
   		$is_display_values = '[{"text":"Да", "value":"1"},{"text":"Нет", "value":"0"}]';
			$is_display_values_text = "";
			foreach(json_decode($is_display_values, true) as $opt)
			{
			  $is_display_values_text.="<option value=\"{$opt['value']}\">{$opt['text']}</option>";
			}
...
$next_order['is_display']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-share-alt"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Социальные сети".' </h2>
...
<div class="genesis-header-property"></div>
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_display&sort_order='. ($next_order['is_display']) .'\' class=\'sort\' column=\'is_display\' sort_order=\''.$sort_order['is_display'].'\'>Отображать'. $sort_icon['is_display'].'</a>
			</div>
					<div class="genesis-header-property"></div>
				</div>
		</div>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=title&sort_order='. ($next_order['title']) .'\' class=\'sort\' column=\'title\' sort_order=\''.$sort_order['title'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['title'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Название:</span>
		</span>".htmlspecialchars($item['title'])."</div>", $item, "Название"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=title&sort_order='. ($next_order['title']) .'\' class=\'sort\' column=\'title\' sort_order=\''.$sort_order['title'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['title'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Название:</span>
		</span>".htmlspecialchars($item['title'])."</div>")."
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
		...
		</span><a target='_blank' class='' href='".addhttp($item['link'])."'>{$item['link']}</a></div>", $item, "Ссылка"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=link&sort_order='. ($next_order['link']) .'\' class=\'sort\' column=\'link\' sort_order=\''.$sort_order['link'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['link'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Ссылка:</span>
		</span><a target='_blank' class='' href='".addhttp($item['link'])."'>{$item['link']}</a></div>")."
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_display&sort_order='. ($next_order['is_display']) .'\' class=\'sort\' column=\'is_display\' sort_order=\''.$sort_order['is_display'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['is_display'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Отображать:</span>
		</span> <span class=''>".renderRadioGroup("is_display", $is_display_values, "social_links", $item['id'], $item['is_display'])."</div>", $item, "Отображать"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_display&sort_order='. ($next_order['is_display']) .'\' class=\'sort\' column=\'is_display\' sort_order=\''.$sort_order['is_display'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['is_display'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Отображать:</span>
		</span> <span class=''>".renderRadioGroup("is_display", $is_display_values, "social_links", $item['id'], $item['is_display'])."</div>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM social_links WHERE id=?",[$id]);
			$item = $item[0];
		}
		...
		$html = '
            ...
            <div class="form-group">
              ...
              <div class="" >'.renderEditRadioGroup("is_display", $is_display_values, $item["is_display"]).'
              </div>
            </div>
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Социальные сети".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `social_links` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO social_links () VALUES ()";
	...
	$actions['create_execute'] = function()
	{
		...
		$params = [];
		$sql = "INSERT INTO social_links () VALUES ()";
...
$set[] = is_null($_REQUEST['is_display'])?"`is_display`=NULL":"`is_display`='".addslashes($_REQUEST['is_display'])."'";

			if(count($set)>0)
			{
				$set = implode(", ", $set);
				$sql = "UPDATE social_links SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM social_links WHERE id=?", [$id]);
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM social_links main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM social_links main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM social_links main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
