<?php
  ...
  0 => '',
	...
	GLOBAL_STORAGE::$parent_object = q1('SELECT * FROM attributes WHERE id = ?', ["{$_REQUEST["attribute_id"]}"]);
...
$next_order['attribute_value']='asc';
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">Значение</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Значения атрибута ".GLOBAL_STORAGE::$parent_object["name"]."".' </h2>
...
<div class="genesis-header-property"></div>
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value&sort_order='. ($next_order['attribute_value']) .'\' class=\'sort\' column=\'attribute_value\' sort_order=\''.$sort_order['attribute_value'].'\'>Значение'. $sort_icon['attribute_value'].'</a>
			</div>
					<div class="genesis-header-property"></div>
				</div>
		</div>
	...
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value&sort_order='. ($next_order['attribute_value']) .'\' class=\'sort\' column=\'attribute_value\' sort_order=\''.$sort_order['attribute_value'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['attribute_value'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Значение:</span>
		</span>
		<span class='editable' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=attributes_values' data-pk='{$item['id']}' data-name='attribute_value'>".htmlspecialchars($item['attribute_value'])."</span>
	</div>", $item, "Значение"):"
	<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value&sort_order='. ($next_order['attribute_value']) .'\' class=\'sort\' column=\'attribute_value\' sort_order=\''.$sort_order['attribute_value'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['attribute_value'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM attributes_values WHERE id=?",[$id]);
			$item = $item[0];
		}
				...
				<fieldset>'.
					...
					.'
								...
								<div class="form-group">
									<label class="control-label" for="textinput">Значение</label>
									<div>
										<input id="attribute_value" name="attribute_value" type="text"  placeholder="" class="form-control input-md " value="'.htmlspecialchars($item["attribute_value"]).'">
									</div>
								</div>

							

					<input id="attribute_id" name="attribute_id" type="hidden" value="'.htmlspecialchars("{$_REQUEST["attribute_id"]}").'">
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Значения атрибута ".GLOBAL_STORAGE::$parent_object["name"]."".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `attributes_values` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO attributes_values (`attribute_value`, `attribute_id`) VALUES (?, ?)";
		...
		foreach($lines as $line)
		{
			...
			qi($sql, [trim($line[0]), "{$_REQUEST["attribute_id"]}"]);
			$last_id = qInsertId();
...
$attribute_id = $_REQUEST['attribute_id'];
		...
		$params = [$attribute_value, $attribute_id];
		$sql = "INSERT INTO attributes_values (`attribute_value`, `attribute_id`) VALUES (?, ?)";
	...
	$actions['edit_execute'] = function()
	{
		...
		if(!$skip)
		{
			...
			$set[] = is_null($_REQUEST['attribute_value'])?"`attribute_value`=NULL":"`attribute_value`='".addslashes($_REQUEST['attribute_value'])."'";
$set[] = is_null($_REQUEST['attribute_id'])?"`attribute_id`=NULL":"`attribute_id`='".addslashes($_REQUEST['attribute_id'])."'";

			if(count($set)>0)
			{
				$set = implode(", ", $set);
				$sql = "UPDATE attributes_values SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM attributes_values WHERE id=?", [$id]);
	...
	function get_agregate()
	{
		...
		$srch = "";
			...
			{
				$srch = "WHERE ((`attribute_value` LIKE '%{$_REQUEST['srch-term']}%'))";
			}

		$filter = filter_query($srch);
		$where = "attribute_id={$_REQUEST["attribute_id"]}";
		if($where != "")
		{
			if($filter!='' || $srch !='')
			{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM attributes_values main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM attributes_values main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM attributes_values main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
	...
	echo masterRender("Значения атрибута ".GLOBAL_STORAGE::$parent_object["name"]."", $content, 6);
