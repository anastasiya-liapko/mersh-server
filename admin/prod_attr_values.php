<?php
  ...
  0 => '',
	...
	$actions[''] = function()
	{
   		...
   		$attribute_value_id_values = json_encode(q("SELECT attribute_value as text, id as value FROM attributes_values WHERE attribute_id='{$_REQUEST["attribute_id"]}'", []));
			$attribute_value_id_values_text = "";
				foreach(json_decode($attribute_value_id_values, true) as $opt)
				{
				  $attribute_value_id_values_text.="<option value=\"{$opt['value']}\">{$opt['text']}</option>";
				}
...
$next_order['id']='asc';
$next_order['attribute_value_id']='asc';
$next_order['price_delta']='asc';
...
$product = q1("SELECT name FROM products WHERE id=(SELECT product_id FROM products_attributes WHERE id=?)", [$_GET['product_attribute_id']]);
$attribute = q1("SELECT name FROM attributes WHERE id=?", [$_GET['attribute_id']]);
		$show = '
		<script>
				...
				<form method="POST">
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<small>Вставьте сюда новые записи. Каждая запись на новой строчке: <b class="csv-create-format">ID, Значения, Изменение цены, Отображать</b></small>
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '.'Значения атрибута "'.$attribute['name'].'" товара '.$product['name'].' </h2>
		...
		$table='
			...
			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value_id&sort_order='. ($next_order['attribute_value_id']) .'\' class=\'sort\' column=\'attribute_value_id\' sort_order=\''.$sort_order['attribute_value_id'].'\'>Значения'. $sort_icon['attribute_value_id'].'</a>
			</div>

			<div class="genesis-header-property">
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=price_delta&sort_order='. ($next_order['price_delta']) .'\' class=\'sort\' column=\'price_delta\' sort_order=\''.$sort_order['price_delta'].'\'>Изменение цены'. $sort_icon['price_delta'].'</a>
			</div>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
				<span class='genesis-attached-column-info'>
					<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value_id&sort_order='. ($next_order['attribute_value_id']) .'\' class=\'sort\' column=\'attribute_value_id\' sort_order=\''.$sort_order['attribute_value_id'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['attribute_value_id'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
					<span class='genesis-attached-column-name'>Значения:</span>
				</span><span class='editable' data-inp='select' data-type='select' data-source='".htmlspecialchars($attribute_value_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=products_attributes_values' data-pk='{$item['id']}' data-name='attribute_value_id'>".select_mapping($attribute_value_id_values, $item['attribute_value_id'])."</span></div>", $item, "Значения"):"<div class='genesis-item-property '>
				<span class='genesis-attached-column-info'>
					<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=attribute_value_id&sort_order='. ($next_order['attribute_value_id']) .'\' class=\'sort\' column=\'attribute_value_id\' sort_order=\''.$sort_order['attribute_value_id'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['attribute_value_id'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
					<span class='genesis-attached-column-name'>Значения:</span>
				</span><span class='editable' data-inp='select' data-type='select' data-source='".htmlspecialchars($attribute_value_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=products_attributes_values' data-pk='{$item['id']}' data-name='attribute_value_id'>".select_mapping($attribute_value_id_values, $item['attribute_value_id'])."</span></div>")."
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=price_delta&sort_order='. ($next_order['price_delta']) .'\' class=\'sort\' column=\'price_delta\' sort_order=\''.$sort_order['price_delta'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['price_delta'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Изменение цены:</span>
		</span><span class='editable' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=products_attributes_values' data-pk='{$item['id']}' data-name='price_delta'>".htmlspecialchars($item['price_delta'])."</span></div>", $item, "Изменение цены"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=price_delta&sort_order='. ($next_order['price_delta']) .'\' class=\'sort\' column=\'price_delta\' sort_order=\''.$sort_order['price_delta'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['price_delta'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Изменение цены:</span>
		</span><span class='editable' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=products_attributes_values' data-pk='{$item['id']}' data-name='price_delta'>".htmlspecialchars($item['price_delta'])."</span></div>")."
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
		...
		</span> <span class=''>".renderRadioGroup("is_visible", $is_visible_values, "products_attributes_values", $item['id'], $item['is_visible'])."</div>", $item, "Отображать"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_visible&sort_order='. ($next_order['is_visible']) .'\' class=\'sort\' column=\'is_visible\' sort_order=\''.$sort_order['is_visible'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['is_visible'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Отображать:</span>
		</span> <span class=''>".renderRadioGroup("is_visible", $is_visible_values, "products_attributes_values", $item['id'], $item['is_visible'])."</div>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM products_attributes_values WHERE id=?",[$id]);
			$item = $item[0];
		}

		
				$attribute_value_id_options = q("SELECT attribute_value as text, id as value FROM attributes_values WHERE attribute_id='{$_REQUEST["attribute_id"]}'",[]);
				$attribute_value_id_options_html = "";
				foreach($attribute_value_id_options as $o)
				{
					$attribute_value_id_options_html .= "<option value=\"{$o['value']}\" ".($o["value"]==$item["attribute_value_id"]?"selected":"").">{$o['text']}</option>";
				}
		...
		$html = '
			<form class="form" enctype="multipart/form-data" method="POST">
				...
				<div class="form-group">
					<label class="control-label" for="textinput">Значения</label>
					<div>
						<select id="attribute_value_id" name="attribute_value_id" class="form-control input-md " >
							'.$attribute_value_id_options_html.'
							</select>
					</div>
				</div>
	               ...
	               <div class="form-group">
	                 <label class="control-label" for="textinput">Изменение цены</label>
	                 <div>
	                   <input id="price_delta" name="price_delta" type="number" step="0.01" class="form-control input-md " placeholder=""  value="'.htmlspecialchars($item["price_delta"]).'">
	                 </div>
	               </div>
            ...
            <div class="form-group">
              ...
              <div class="" >'.renderEditRadioGroup("is_visible", $is_visible_values, $item["is_visible"]).'
					...
					<input id="product_attribute_id" name="product_attribute_id" type="hidden" value="'.htmlspecialchars("{$_REQUEST["product_attribute_id"]}").'">
...
{"text":"Нет", "value":"0"}]';
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Значения атрибута товара".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `products_attributes_values` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO products_attributes_values (`attribute_value_id`, `price_delta`, `is_visible`, `product_attribute_id`) VALUES (?, ?, ?, ?)";
		...
		foreach($lines as $line)
		{
			...
			qi($sql, [trim($line[0]), trim($line[1]), trim($line[2]), trim($line[3]), "{$_REQUEST["product_attribute_id"]}"]);
			$last_id = qInsertId();
	...
	$actions['create_execute'] = function()
	{
		...
		$attribute_value_id = $_REQUEST['attribute_value_id'];
$price_delta = $_REQUEST['price_delta'];
$is_visible = $_REQUEST['is_visible'];
$product_attribute_id = $_REQUEST['product_attribute_id'];

		$params = [$attribute_value_id, $price_delta, $is_visible, $product_attribute_id];
		$sql = "INSERT INTO products_attributes_values (`attribute_value_id`, `price_delta`, `is_visible`, `product_attribute_id`) VALUES (?, ?, ?, ?)";
	...
	$actions['edit_execute'] = function()
	{
		...
		if(!$skip)
		{
			...
			$set[] = is_null($_REQUEST['attribute_value_id'])?"`attribute_value_id`=NULL":"`attribute_value_id`='".addslashes($_REQUEST['attribute_value_id'])."'";
$set[] = is_null($_REQUEST['price_delta'])?"`price_delta`=NULL":"`price_delta`='".addslashes($_REQUEST['price_delta'])."'";
$set[] = is_null($_REQUEST['is_visible'])?"`is_visible`=NULL":"`is_visible`='".addslashes($_REQUEST['is_visible'])."'";
$set[] = is_null($_REQUEST['product_attribute_id'])?"`product_attribute_id`=NULL":"`product_attribute_id`='".addslashes($_REQUEST['product_attribute_id'])."'";

			if(count($set)>0)
			{
				$set = implode(", ", $set);
				$sql = "UPDATE products_attributes_values SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM products_attributes_values WHERE id=?", [$id]);
...
{"text":"Нет", "value":"0"}]';
	...
	function get_agregate()
	{
		...
		$where = "product_attribute_id={$_REQUEST["product_attribute_id"]}";
		if($where != "")
		{
			if($filter!='' || $srch !='')
			{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM products_attributes_values main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM products_attributes_values main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM products_attributes_values main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
	...
	echo masterRender("Значения атрибута товара", $content, 7);
