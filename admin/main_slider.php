...
$next_order['is_visible']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-images"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Слайдер на главной".' </h2>
...
<div class="genesis-header-property"></div>
		...
		if(count($items) > 0)
		{
			...
			foreach($items as $item)
			{
				...
				if ($item['is_video'] == '1') {
					...
					$media_file = "<video class='genesis-video' style='max-width:200px; max-height:200px;' controls><source src='".$item["content"]."'></video>";
		...
		<span class='genesis-attached-column-info'>
			...
			<span class='genesis-attached-column-name'>Медиафайл:</span>
					...
					".$media_file."
				</div>", $item, "Медиафайл"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=content&sort_order='. ($next_order['content']) .'\' class=\'sort\' column=\'content\' sort_order=\''.$sort_order['content'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['content'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Медиафайл:</span>
		</span>
...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		...
		</span> <span class=''>".renderRadioGroup("is_video", $is_video_values, "main_slider", $item['id'], $item['is_video'])."</div>", $item, "Тип"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_video&sort_order='. ($next_order['is_video']) .'\' class=\'sort\' column=\'is_video\' sort_order=\''.$sort_order['is_video'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['is_video'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Тип:</span>
		</span> <span class=''>".renderRadioGroup("is_video", $is_video_values, "main_slider", $item['id'], $item['is_video'])."</div>")."
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
		...
		</span> <span class=''>".renderRadioGroup("is_visible", $is_visible_values, "main_slider", $item['id'], $item['is_visible'])."</div>", $item, "Отображать"):"<div class='genesis-item-property '>
		<span class='genesis-attached-column-info'>
			<span class='buttons-panel'>".'<a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=is_visible&sort_order='. ($next_order['is_visible']) .'\' class=\'sort\' column=\'is_visible\' sort_order=\''.$sort_order['is_visible'].'\'>'. (str_replace('style="margin-left:5px;"','',$sort_icon['is_visible'] ?? '<span class="fa fa-sort"></span>')).'</a>'."</span>
			<span class='genesis-attached-column-name'>Отображать:</span>
		</span> <span class=''>".renderRadioGroup("is_visible", $is_visible_values, "main_slider", $item['id'], $item['is_visible'])."</div>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM main_slider WHERE id=?",[$id]);
			$item = $item[0];
		}
...
{"text":"Нет", "value":"0"}]';
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Слайдер на главной".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `main_slider` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['csv_create_execute'] = function()
	{
		...
		$sql = "INSERT IGNORE INTO main_slider (`content`, `is_video`, `is_visible`) VALUES (?, ?, ?)";
...
$is_visible = $_REQUEST['is_visible'];
		...
		$params = [$content, $is_video, $is_visible];
		$sql = "INSERT INTO main_slider (`content`, `is_video`, `is_visible`) VALUES (?, ?, ?)";
...
$set[] = is_null($_REQUEST['is_visible'])?"`is_visible`=NULL":"`is_visible`='".addslashes($_REQUEST['is_visible'])."'";
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE main_slider SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM main_slider WHERE id=?", [$id]);
...
{"text":"Нет", "value":"0"}]';
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.*  FROM main_slider main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM main_slider main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM main_slider main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
	...
	echo masterRender("Слайдер на главной", $content, 17);
